...
class WifiReal(WifiAPI):
    ...
    def prt(self, msg):
        ...
        print("{}: Wifi {}: {}".format(now, self.mac, msg))
    ...
    def setup(self):
        ...
        # get mac
        result = safe_dec(self.serial.write_cmd("AT+CIPSTAMAC_CUR?"))
    ...
    def send_data(self, dest_mac, data):
        ...
        if dest_mac in self.direct_conns:
            # Translate to linkId
            self.serial.write_data(self.direct_conns[dest_mac], "{}\n".format(data))
            return True
        elif dest_mac == self.connected_ap[1]:
            # Translate to linkId
            self.serial.write_data(self.connected_ap[0], "{}\n".format(data))
            return True
        return False
    ...
    def process_data(self, linkId, data):
        ...
        if packets[0] == "CS":
            ...
            self.serial.write_data(linkId, "\n")
            self.serial.write_data(linkId, "CA#{}\n".format(self.mac))
    ...
    def close_conn(self, linkId):
        ...
        # TODO remove everything (dir_conn, call handler)
    ...
    def get_direct_connections(self):
        ...
        if self.connected_ap and self.connect_to_ap not in conns:
